...
| 24 | Windows raw-device UAC-elevation feasibility | Implemented via PR #49 (`feat/m0-d24-windows`, frozen SHA `f465ac3`) on the operator's real Windows 10 Pro hardware (human-operated relay, no direct network path from this session) — `crates/trareon-core/src/platform.rs` `windows::probe_physical_drive_zero()`, `docs/platform/day24-windows-feasibility.md`. Finding: raw-disk access gated by UAC elevation state, not Administrators-group membership alone | Only handle-open feasibility tested (no read/write); ThinkPad X270 (the dedicated lab machine per `docs/AI-DEVELOPMENT-WORKFLOW.md`) was not used, so lower-spec/older-Windows behavior remains `NotValidated` |
| 25 | macOS raw-device feasibility | Not started | `MANUAL_START` + physical device required |
