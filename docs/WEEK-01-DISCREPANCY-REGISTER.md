...
| 24–25 | Windows/macOS raw-device feasibility | Day 25 implemented on M4 Pro Mac (see `docs/platform/day25-macos-feasibility.md`); Day 24 not started | Day 24 still `MANUAL_START`; macOS: raw open denied without `operator`/helper; Intel/`hdiutil` attach `NotValidated` |
