# Day 02 — Workspace dan core boundary
...
- Prasyarat: Day 01 dengan independent review EXPECTED_PASS.
- Project root: `/Users/user/Projects/Trareon/Trareon Acquire`.
- Branch: `feat/m0-d02-workspace`; satu author dan satu worktree aktif.
...
- [ ] Pastikan Issue Day 02 berada di status yang benar dan tidak ada Day lain menulis file yang sama.
...
Acceptance focus: core bebas Tauri/frontend, dependency exact, failing test dibuktikan sebelum implementasi.
...
- Verification: `cargo test --workspace --all-targets; cargo fmt --all -- --check; cargo clippy --workspace --all-targets -- -D warnings`.
...
```text
Anda adalah independent reviewer Codex untuk M0-D02. Jangan menulis pada worktree author. Baca RFC, active runbook, commit_before, frozen commit_after, diff, test logs, dan local evidence.

Verifikasi sendiri scope, correctness, negative paths, false-complete resistance, forensic validity, security, dependency boundary, deterministic behavior, performance equivalence, documentation, dan test oracle. Fokus khusus: core bebas Tauri/frontend, dependency exact, failing test dibuktikan sebelum implementasi.
...
Jika semua exit checklist lulus, handoff ke `DAY-03.md` dengan frozen SHA, TaskResult.v1 author/reviewer/Antigravity, CI/platform evidence, incident/recovery count, dan remaining risks. Selain itu tetap pada Day 02.
