# Day 07 — Week 1 integration gate

## Outcome

Integrasikan Days 01-06, jalankan clean checks dua kali, periksa dependency boundary, dan buat discrepancy register.

## Entry Gate

- Prasyarat: Day 06 dengan independent review EXPECTED_PASS.
- Project root: `/Users/user/Projects/Trareon/Trareon Acquire`.
- Branch: `feat/m0-d07-integration`; satu author dan satu worktree aktif.
...
- [ ] Pastikan Issue Day 07 berada di status yang benar dan tidak ada Day lain menulis file yang sama.
...
```text
Anda adalah author untuk M0-D07 Trareon Acquire.

Outcome: Integrasikan Days 01-06, jalankan clean checks dua kali, periksa dependency boundary, dan buat discrepancy register.
Scope file/interface: workspace; Cargo.lock; Week 1 evidence and discrepancy report.
Branch wajib: feat/m0-d07-integration. Baca authority files dan existing code sebelum perubahan. Gunakan TDD: tulis test yang gagal dengan alasan yang diharapkan, jalankan dan catat exit code, implementasikan perubahan minimum, lalu jalankan verification lengkap.

Verification wajib: cargo test --workspace --all-targets; cargo test --workspace --all-targets; cargo fmt --all -- --check; cargo clippy --workspace --all-targets -- -D warnings.
Acceptance focus: dua run identik, no P0/P1, diff tetap sesuai scope, baseline RFC tidak berubah.
...
- Expected files/interfaces: workspace; Cargo.lock; Week 1 evidence and discrepancy report.
- Expected behavior: Integrasikan Days 01-06, jalankan clean checks dua kali, periksa dependency boundary, dan buat discrepancy register.
- Explicit non-goals: fitur setelah Day 07, production claim, raw/elevated operation tanpa gate, refactor di luar scope, dan perubahan RFC.
- Diff di luar map menjadi `FIX + SCOPE-DRIFT` sampai dijelaskan dan disetujui.

## Expected Result

- Verification: `cargo test --workspace --all-targets; cargo test --workspace --all-targets; cargo fmt --all -- --check; cargo clippy --workspace --all-targets -- -D warnings`.
...
```text
Anda adalah independent reviewer Codex untuk M0-D07. Jangan menulis pada worktree author. Baca RFC, active runbook, commit_before, frozen commit_after, diff, test logs, dan local evidence.

Verifikasi sendiri scope, correctness, negative paths, false-complete resistance, forensic validity, security, dependency boundary, deterministic behavior, performance equivalence, documentation, dan test oracle. Fokus khusus: dua run identik, no P0/P1, diff tetap sesuai scope, baseline RFC tidak berubah.

Jalankan command aman yang relevan: cargo test --workspace --all-targets; cargo test --workspace --all-targets; cargo fmt --all -- --check; cargo clippy --workspace --all-targets -- -D warnings. Bandingkan exact local/remote/PR/CI SHA bila remote evidence tersedia. SHA mismatch adalah BLOCKED + REMOTE-STATE-DIVERGENCE. Hasil tanpa reproducible evidence adalah UNVERIFIED.
...
Jika semua exit checklist lulus, handoff ke `DAY-08.md` dengan frozen SHA, TaskResult.v1 author/reviewer/Antigravity, CI/platform evidence, incident/recovery count, dan remaining risks. Selain itu tetap pada Day 07.
