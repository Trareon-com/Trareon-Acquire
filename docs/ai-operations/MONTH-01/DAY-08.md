# Day 08 — Synthetic streaming acquisition

## Outcome

Implementasikan file-backed streaming acquisition dari Foundation Plan Task 4 dengan byte-for-byte oracle dan streaming SHA-256.

## Entry Gate

- Prasyarat: Day 07 dengan independent review EXPECTED_PASS.
- Project root: `/Users/user/Projects/Trareon/Trareon Acquire`.
- Branch: `feat/m0-d08-streaming`; satu author dan satu worktree aktif.
...
- [ ] Pastikan Issue Day 08 berada di status yang benar dan tidak ada Day lain menulis file yang sama.
...
```text
Anda adalah author untuk M0-D08 Trareon Acquire.

Outcome: Implementasikan file-backed streaming acquisition dari Foundation Plan Task 4 dengan byte-for-byte oracle dan streaming SHA-256.
Scope file/interface: crates/traeron-core/src/acquisition.rs; crates/traeron-core/tests/acquisition.rs; crates/traeron-core/src/lib.rs.
Branch wajib: feat/m0-d08-streaming. Baca authority files dan existing code sebelum perubahan. Gunakan TDD: tulis test yang gagal dengan alasan yang diharapkan, jalankan dan catat exit code, implementasikan perubahan minimum, lalu jalankan verification lengkap.

Verification wajib: cargo test -p traeron-core --test acquisition; cargo test --workspace --all-targets.
Acceptance focus: output byte-identik, hash oracle independen, memory tidak membaca seluruh source sekaligus.
...
- Expected files/interfaces: crates/traeron-core/src/acquisition.rs; crates/traeron-core/tests/acquisition.rs; crates/traeron-core/src/lib.rs.
- Expected behavior: Implementasikan file-backed streaming acquisition dari Foundation Plan Task 4 dengan byte-for-byte oracle dan streaming SHA-256.
- Explicit non-goals: fitur setelah Day 08, production claim, raw/elevated operation tanpa gate, refactor di luar scope, dan perubahan RFC.
- Diff di luar map menjadi `FIX + SCOPE-DRIFT` sampai dijelaskan dan disetujui.

## Expected Result

- Verification: `cargo test -p traeron-core --test acquisition; cargo test --workspace --all-targets`.
...
```text
Anda adalah independent reviewer Codex untuk M0-D08. Jangan menulis pada worktree author. Baca RFC, active runbook, commit_before, frozen commit_after, diff, test logs, dan local evidence.

Verifikasi sendiri scope, correctness, negative paths, false-complete resistance, forensic validity, security, dependency boundary, deterministic behavior, performance equivalence, documentation, dan test oracle. Fokus khusus: output byte-identik, hash oracle independen, memory tidak membaca seluruh source sekaligus.

Jalankan command aman yang relevan: cargo test -p traeron-core --test acquisition; cargo test --workspace --all-targets. Bandingkan exact local/remote/PR/CI SHA bila remote evidence tersedia. SHA mismatch adalah BLOCKED + REMOTE-STATE-DIVERGENCE. Hasil tanpa reproducible evidence adalah UNVERIFIED.
...
Jika semua exit checklist lulus, handoff ke `DAY-09.md` dengan frozen SHA, TaskResult.v1 author/reviewer/Antigravity, CI/platform evidence, incident/recovery count, dan remaining risks. Selain itu tetap pada Day 08.
