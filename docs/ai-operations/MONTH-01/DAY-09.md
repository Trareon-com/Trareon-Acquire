# Day 09 — Source destination identity guard

## Outcome

Tambahkan preflight identity, same-source/destination rejection, alias/symlink containment, dan no-output-on-failed-preflight.

## Entry Gate

- Prasyarat: Day 08 dengan independent review EXPECTED_PASS.
- Project root: `/Users/user/Projects/Trareon/Trareon Acquire`.
- Branch: `feat/m0-d09-identity`; satu author dan satu worktree aktif.
...
- [ ] Pastikan Issue Day 09 berada di status yang benar dan tidak ada Day lain menulis file yang sama.
...
```text
Anda adalah author untuk M0-D09 Trareon Acquire.

Outcome: Tambahkan preflight identity, same-source/destination rejection, alias/symlink containment, dan no-output-on-failed-preflight.
Scope file/interface: crates/traeron-core/src/acquisition.rs; crates/traeron-core/tests/acquisition.rs.
Branch wajib: feat/m0-d09-identity. Baca authority files dan existing code sebelum perubahan. Gunakan TDD: tulis test yang gagal dengan alasan yang diharapkan, jalankan dan catat exit code, implementasikan perubahan minimum, lalu jalankan verification lengkap.

Verification wajib: cargo test -p traeron-core --test acquisition -- --nocapture.
Acceptance focus: reversal/alias ditolak sebelum write dan rejection diaudit.
...
- Expected behavior: Tambahkan preflight identity, same-source/destination rejection, alias/symlink containment, dan no-output-on-failed-preflight.
- Explicit non-goals: fitur setelah Day 09, production claim, raw/elevated operation tanpa gate, refactor di luar scope, dan perubahan RFC.
...
```text
Anda adalah independent reviewer Codex untuk M0-D09. Jangan menulis pada worktree author. Baca RFC, active runbook, commit_before, frozen commit_after, diff, test logs, dan local evidence.

Verifikasi sendiri scope, correctness, negative paths, false-complete resistance, forensic validity, security, dependency boundary, deterministic behavior, performance equivalence, documentation, dan test oracle. Fokus khusus: reversal/alias ditolak sebelum write dan rejection diaudit.
...
Jika semua exit checklist lulus, handoff ke `DAY-10.md` dengan frozen SHA, TaskResult.v1 author/reviewer/Antigravity, CI/platform evidence, incident/recovery count, dan remaining risks. Selain itu tetap pada Day 09.
