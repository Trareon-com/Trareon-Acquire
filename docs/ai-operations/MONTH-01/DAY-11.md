...
Verification wajib: cargo test -p trareon-core --test acquisition -- --nocapture.
...
- Verification: `cargo test -p trareon-core --test acquisition -- --nocapture`.
...
Jalankan command aman yang relevan: cargo test -p trareon-core --test acquisition -- --nocapture. Bandingkan exact local/remote/PR/CI SHA bila remote evidence tersedia. SHA mismatch adalah BLOCKED + REMOTE-STATE-DIVERGENCE. Hasil tanpa reproducible evidence adalah UNVERIFIED.
