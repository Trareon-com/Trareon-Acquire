# Day 11 — RAW dan split-RAW contract

## Outcome

Implementasikan RAW dan split-RAW file-backed output dengan boundary-size, segment-order, final-short-segment, zero-length, dan reassembly tests.

## Entry Gate

- Prasyarat: Day 10 dengan independent review EXPECTED_PASS.
- Project root: `/Users/user/Projects/Trareon/Trareon Acquire`.
- Branch: `feat/m0-d11-raw`; satu author dan satu worktree aktif.
...
- [ ] Pastikan Issue Day 11 berada di status yang benar dan tidak ada Day lain menulis file yang sama.
...
```text
Anda adalah author untuk M0-D11 Trareon Acquire.

Outcome: Implementasikan RAW dan split-RAW file-backed output dengan boundary-size, segment-order, final-short-segment, zero-length, dan reassembly tests.
Scope file/interface: crates/traeron-core/src/acquisition.rs; crates/traeron-core/tests/acquisition.rs.
Branch wajib: feat/m0-d11-raw. Baca authority files dan existing code sebelum perubahan. Gunakan TDD: tulis test yang gagal dengan alasan yang diharapkan, jalankan dan catat exit code, implementasikan perubahan minimum, lalu jalankan verification lengkap.

Verification wajib: cargo test -p traeron-core --test acquisition -- --nocapture.
Acceptance focus: reassembly byte-identik dan segment metadata deterministik.
...
- Expected files/interfaces: crates/traeron-core/src/acquisition.rs; crates/traeron-core/tests/acquisition.rs.
- Expected behavior: Implementasikan RAW dan split-RAW file-backed output dengan boundary-size, segment-order, final-short-segment, zero-length, dan reassembly tests.
- Explicit non-goals: fitur setelah Day 11, production claim, raw/elevated operation tanpa gate, refactor di luar scope, dan perubahan RFC.
- Diff di luar map menjadi `FIX + SCOPE-DRIFT` sampai dijelaskan dan disetujui.

## Expected Result

- Verification: `cargo test -p traeron-core --test acquisition -- --nocapture`.
...
```text
Anda adalah independent reviewer Codex untuk M0-D11. Jangan menulis pada worktree author. Baca RFC, active runbook, commit_before, frozen commit_after, diff, test logs, dan local evidence.

Verifikasi sendiri scope, correctness, negative paths, false-complete resistance, forensic validity, security, dependency boundary, deterministic behavior, performance equivalence, documentation, dan test oracle. Fokus khusus: reassembly byte-identik dan segment metadata deterministik.

Jalankan command aman yang relevan: cargo test -p traeron-core --test acquisition -- --nocapture. Bandingkan exact local/remote/PR/CI SHA bila remote evidence tersedia. SHA mismatch adalah BLOCKED + REMOTE-STATE-DIVERGENCE. Hasil tanpa reproducible evidence adalah UNVERIFIED.
...
Jika semua exit checklist lulus, handoff ke `DAY-12.md` dengan frozen SHA, TaskResult.v1 author/reviewer/Antigravity, CI/platform evidence, incident/recovery count, dan remaining risks. Selain itu tetap pada Day 11.
