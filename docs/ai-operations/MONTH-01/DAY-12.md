# Day 12 — fsnap manifest schema

## Outcome

Definisikan schema/version, source identity, segments, hashes, coverage, gaps, audit reference, tool identity, dan validation errors.

## Entry Gate

- Prasyarat: Day 11 dengan independent review EXPECTED_PASS.
- Project root: `/Users/user/Projects/Trareon/Trareon Acquire`.
- Branch: `feat/m0-d12-manifest`; satu author dan satu worktree aktif.
...
- [ ] Pastikan Issue Day 12 berada di status yang benar dan tidak ada Day lain menulis file yang sama.
...
```text
Anda adalah author untuk M0-D12 Trareon Acquire.

Outcome: Definisikan schema/version, source identity, segments, hashes, coverage, gaps, audit reference, tool identity, dan validation errors.
Scope file/interface: schemas/fsnap-manifest-v1.schema.json; crates/traeron-core/src/package.rs; crates/traeron-core/tests/package.rs.
Branch wajib: feat/m0-d12-manifest. Baca authority files dan existing code sebelum perubahan. Gunakan TDD: tulis test yang gagal dengan alasan yang diharapkan, jalankan dan catat exit code, implementasikan perubahan minimum, lalu jalankan verification lengkap.

Verification wajib: cargo test -p traeron-core --test package; cargo test --workspace --all-targets.
Acceptance focus: schema menolak required-field loss, invalid path/hash/version dan tidak mengklaim complete tanpa coverage.
...
- Expected files/interfaces: schemas/fsnap-manifest-v1.schema.json; crates/traeron-core/src/package.rs; crates/traeron-core/tests/package.rs.
- Expected behavior: Definisikan schema/version, source identity, segments, hashes, coverage, gaps, audit reference, tool identity, dan validation errors.
- Explicit non-goals: fitur setelah Day 12, production claim, raw/elevated operation tanpa gate, refactor di luar scope, dan perubahan RFC.
- Diff di luar map menjadi `FIX + SCOPE-DRIFT` sampai dijelaskan dan disetujui.

## Expected Result

- Verification: `cargo test -p traeron-core --test package; cargo test --workspace --all-targets`.
...
```text
Anda adalah independent reviewer Codex untuk M0-D12. Jangan menulis pada worktree author. Baca RFC, active runbook, commit_before, frozen commit_after, diff, test logs, dan local evidence.

Verifikasi sendiri scope, correctness, negative paths, false-complete resistance, forensic validity, security, dependency boundary, deterministic behavior, performance equivalence, documentation, dan test oracle. Fokus khusus: schema menolak required-field loss, invalid path/hash/version dan tidak mengklaim complete tanpa coverage.

Jalankan command aman yang relevan: cargo test -p traeron-core --test package; cargo test --workspace --all-targets. Bandingkan exact local/remote/PR/CI SHA bila remote evidence tersedia. SHA mismatch adalah BLOCKED + REMOTE-STATE-DIVERGENCE. Hasil tanpa reproducible evidence adalah UNVERIFIED.
...
Jika semua exit checklist lulus, handoff ke `DAY-13.md` dengan frozen SHA, TaskResult.v1 author/reviewer/Antigravity, CI/platform evidence, incident/recovery count, dan remaining risks. Selain itu tetap pada Day 12.
