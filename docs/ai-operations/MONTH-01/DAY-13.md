# Day 13 — Safe fsnap package writer

## Outcome

Implementasikan safe package writer dari Foundation Plan Task 5 dengan traversal, duplicate, missing, mutation, dan audit-discontinuity fixtures.

## Entry Gate

- Prasyarat: Day 12 dengan independent review EXPECTED_PASS.
- Project root: `/Users/user/Projects/Trareon/Trareon Acquire`.
- Branch: `feat/m0-d13-package`; satu author dan satu worktree aktif.
...
- [ ] Pastikan Issue Day 13 berada di status yang benar dan tidak ada Day lain menulis file yang sama.
...
```text
Anda adalah author untuk M0-D13 Trareon Acquire.

Outcome: Implementasikan safe package writer dari Foundation Plan Task 5 dengan traversal, duplicate, missing, mutation, dan audit-discontinuity fixtures.
Scope file/interface: crates/traeron-core/src/package.rs; crates/traeron-core/tests/package.rs; schemas/fsnap-manifest-v1.schema.json.
Branch wajib: feat/m0-d13-package. Baca authority files dan existing code sebelum perubahan. Gunakan TDD: tulis test yang gagal dengan alasan yang diharapkan, jalankan dan catat exit code, implementasikan perubahan minimum, lalu jalankan verification lengkap.

Verification wajib: cargo test -p traeron-core --test package -- --nocapture.
Acceptance focus: path containment fail-closed, package partial tidak dianggap verified.
...
- Expected files/interfaces: crates/traeron-core/src/package.rs; crates/traeron-core/tests/package.rs; schemas/fsnap-manifest-v1.schema.json.
- Expected behavior: Implementasikan safe package writer dari Foundation Plan Task 5 dengan traversal, duplicate, missing, mutation, dan audit-discontinuity fixtures.
- Explicit non-goals: fitur setelah Day 13, production claim, raw/elevated operation tanpa gate, refactor di luar scope, dan perubahan RFC.
- Diff di luar map menjadi `FIX + SCOPE-DRIFT` sampai dijelaskan dan disetujui.

## Expected Result

- Verification: `cargo test -p traeron-core --test package -- --nocapture`.
...
```text
Anda adalah independent reviewer Codex untuk M0-D13. Jangan menulis pada worktree author. Baca RFC, active runbook, commit_before, frozen commit_after, diff, test logs, dan local evidence.

Verifikasi sendiri scope, correctness, negative paths, false-complete resistance, forensic validity, security, dependency boundary, deterministic behavior, performance equivalence, documentation, dan test oracle. Fokus khusus: path containment fail-closed, package partial tidak dianggap verified.

Jalankan command aman yang relevan: cargo test -p traeron-core --test package -- --nocapture. Bandingkan exact local/remote/PR/CI SHA bila remote evidence tersedia. SHA mismatch adalah BLOCKED + REMOTE-STATE-DIVERGENCE. Hasil tanpa reproducible evidence adalah UNVERIFIED.
...
Jika semua exit checklist lulus, handoff ke `DAY-14.md` dengan frozen SHA, TaskResult.v1 author/reviewer/Antigravity, CI/platform evidence, incident/recovery count, dan remaining risks. Selain itu tetap pada Day 13.
