...
Implementasikan Tauri/Svelte shell dari Foundation Plan Task 7; UI hanya memanggil typed commands dan core menentukan final state.

## Entry Gate

- Prasyarat: Day 15 dengan independent review EXPECTED_PASS.
- Project root: `/Users/user/Projects/Trareon/Trareon Acquire`.
- Branch: `feat/m0-d16-tauri`; satu author dan satu worktree aktif.
...
- [ ] Pastikan Issue Day 16 berada di status yang benar dan tidak ada Day lain menulis file yang sama.
...
Acceptance focus: main UI tidak elevated, no shell/arbitrary filesystem, error/state mapping lengkap.
...
- Verification: `cargo test --workspace --all-targets; npm ci; npm run check; npm run build`.
...
```text
Anda adalah independent reviewer Codex untuk M0-D16. Jangan menulis pada worktree author. Baca RFC, active runbook, commit_before, frozen commit_after, diff, test logs, dan local evidence.

Verifikasi sendiri scope, correctness, negative paths, false-complete resistance, forensic validity, security, dependency boundary, deterministic behavior, performance equivalence, documentation, dan test oracle. Fokus khusus: main UI tidak elevated, no shell/arbitrary filesystem, error/state mapping lengkap.

Jalankan command aman yang relevan: cargo test --workspace --all-targets; npm ci; npm run check; npm run build. Bandingkan exact local/remote/PR/CI SHA bila remote evidence tersedia. SHA mismatch adalah BLOCKED + REMOTE-STATE-DIVERGENCE. Hasil tanpa reproducible evidence adalah UNVERIFIED.
...
Jika semua exit checklist lulus, handoff ke `DAY-17.md` dengan frozen SHA, TaskResult.v1 author/reviewer/Antigravity, CI/platform evidence, incident/recovery count, dan remaining risks. Selain itu tetap pada Day 16.
