...
# Day 19 — Chain of Custody dan report preview

## Outcome

Render CoC/report preview hanya dari core data dan bedakan verified, incomplete, failed, cancelled, serta NotValidated.

## Entry Gate

- Prasyarat: Day 18 dengan independent review EXPECTED_PASS.
- Project root: `/Users/user/Projects/Trareon/Trareon Acquire`.
- Branch: `feat/m0-d19-report`; satu author dan satu worktree aktif.
...
- [ ] Pastikan Issue Day 19 berada di status yang benar dan tidak ada Day lain menulis file yang sama.
...
```text
Anda adalah author untuk M0-D19 Trareon Acquire.

Outcome: Render CoC/report preview hanya dari core data dan bedakan verified, incomplete, failed, cancelled, serta NotValidated.
Scope file/interface: core report types; typed command; report UI and tests.
Branch wajib: feat/m0-d19-report. Baca authority files dan existing code sebelum perubahan. Gunakan TDD: tulis test yang gagal dengan alasan yang diharapkan, jalankan dan catat exit code, implementasikan perubahan minimum, lalu jalankan verification lengkap.

Verification wajib: cargo test --workspace --all-targets; npm run check; npm run build.
Acceptance focus: report provenance exact, no UI-only fields, limitation terlihat.
...
- Expected files/interfaces: core report types; typed command; report UI and tests.
- Expected behavior: Render CoC/report preview hanya dari core data dan bedakan verified, incomplete, failed, cancelled, serta NotValidated.
- Explicit non-goals: fitur setelah Day 19, production claim, raw/elevated operation tanpa gate, refactor di luar scope, dan perubahan RFC.
- Diff di luar map menjadi `FIX + SCOPE-DRIFT` sampai dijelaskan dan disetujui.

## Expected Result

- Verification: `cargo test --workspace --all-targets; npm run check; npm run build`.
...
```text
Anda adalah independent reviewer Codex untuk M0-D19. Jangan menulis pada worktree author. Baca RFC, active runbook, commit_before, frozen commit_after, diff, test logs, dan local evidence.

Verifikasi sendiri scope, correctness, negative paths, false-complete resistance, forensic validity, security, dependency boundary, deterministic behavior, performance equivalence, documentation, dan test oracle. Fokus khusus: report provenance exact, no UI-only fields, limitation terlihat.

Jalankan command aman yang relevan: cargo test --workspace --all-targets; npm run check; npm run build. Bandingkan exact local/remote/PR/CI SHA bila remote evidence tersedia. SHA mismatch adalah BLOCKED + REMOTE-STATE-DIVERGENCE. Hasil tanpa reproducible evidence adalah UNVERIFIED.
...
```text
Validasi frozen SHA untuk mode VALIDATE_REPORT. Jangan mengubah core semantics. Uji alur, keyboard/focus, error/limitation visibility, viewport, dan copy yang relevan. Simpan screenshot teredaksi tanpa data pribadi. Laporkan mismatch sebagai FIX; jangan memperbaiki branch author. Akhiri dengan TaskResult.v1.
...
Jika semua exit checklist lulus, handoff ke `DAY-20.md` dengan frozen SHA, TaskResult.v1 author/reviewer/Antigravity, CI/platform evidence, incident/recovery count, dan remaining risks. Selain itu tetap pada Day 19.
