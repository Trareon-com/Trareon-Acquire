# Day 21 — DevSecOps gates

## Outcome

Tambahkan secret, dependency, license, SAST, SBOM, artifact, provenance-intent, dan policy checks tanpa auto-release.

## Entry Gate

- Prasyarat: Day 20 dengan independent review EXPECTED_PASS.
- Project root: `/Users/user/Projects/Trareon/Trareon Acquire`.
- Branch: `feat/m0-d21-devsecops`; satu author dan satu worktree aktif.
...
- [ ] Pastikan Issue Day 21 berada di status yang benar dan tidak ada Day lain menulis file yang sama.
...
```text
Anda adalah author untuk M0-D21 Trareon Acquire.

Outcome: Tambahkan secret, dependency, license, SAST, SBOM, artifact, provenance-intent, dan policy checks tanpa auto-release.
Scope file/interface: CI security jobs; deny/license policy; SBOM config; SECURITY.md.
Branch wajib: feat/m0-d21-devsecops. Baca authority files dan existing code sebelum perubahan. Gunakan TDD: tulis test yang gagal dengan alasan yang diharapkan, jalankan dan catat exit code, implementasikan perubahan minimum, lalu jalankan verification lengkap.

Verification wajib: cargo test --workspace --all-targets; npm audit --omit=dev; sh scripts/validate-ai-operations.sh.
Acceptance focus: finding tidak disembunyikan, dependency baru high-risk meminta human approval.
...
- Expected files/interfaces: CI security jobs; deny/license policy; SBOM config; SECURITY.md.
- Expected behavior: Tambahkan secret, dependency, license, SAST, SBOM, artifact, provenance-intent, dan policy checks tanpa auto-release.
- Explicit non-goals: fitur setelah Day 21, production claim, raw/elevated operation tanpa gate, refactor di luar scope, dan perubahan RFC.
- Diff di luar map menjadi `FIX + SCOPE-DRIFT` sampai dijelaskan dan disetujui.

## Expected Result

- Verification: `cargo test --workspace --all-targets; npm audit --omit=dev; sh scripts/validate-ai-operations.sh`.
...
```text
Anda adalah independent reviewer Codex untuk M0-D21. Jangan menulis pada worktree author. Baca RFC, active runbook, commit_before, frozen commit_after, diff, test logs, dan local evidence.

Verifikasi sendiri scope, correctness, negative paths, false-complete resistance, forensic validity, security, dependency boundary, deterministic behavior, performance equivalence, documentation, dan test oracle. Fokus khusus: finding tidak disembunyikan, dependency baru high-risk meminta human approval.

Jalankan command aman yang relevan: cargo test --workspace --all-targets; npm audit --omit=dev; sh scripts/validate-ai-operations.sh. Bandingkan exact local/remote/PR/CI SHA bila remote evidence tersedia. SHA mismatch adalah BLOCKED + REMOTE-STATE-DIVERGENCE. Hasil tanpa reproducible evidence adalah UNVERIFIED.
...
Jika semua exit checklist lulus, handoff ke `DAY-22.md` dengan frozen SHA, TaskResult.v1 author/reviewer/Antigravity, CI/platform evidence, incident/recovery count, dan remaining risks. Selain itu tetap pada Day 21.
