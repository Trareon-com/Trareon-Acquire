# Day 22 — Property dan fuzz baseline

## Outcome

Tambahkan bounded property/fuzz targets untuk state, canonical path, manifest, package containment, audit continuity, dan verifier input.

## Entry Gate

- Prasyarat: Day 21 dengan independent review EXPECTED_PASS.
- Project root: `/Users/user/Projects/Trareon/Trareon Acquire`.
- Branch: `feat/m0-d22-fuzz`; satu author dan satu worktree aktif.
...
- [ ] Pastikan Issue Day 22 berada di status yang benar dan tidak ada Day lain menulis file yang sama.
...
```text
Anda adalah author untuk M0-D22 Trareon Acquire.

Outcome: Tambahkan bounded property/fuzz targets untuk state, canonical path, manifest, package containment, audit continuity, dan verifier input.
Scope file/interface: core property tests; fuzz targets/corpus documentation; CI bounded fuzz smoke.
Branch wajib: feat/m0-d22-fuzz. Baca authority files dan existing code sebelum perubahan. Gunakan TDD: tulis test yang gagal dengan alasan yang diharapkan, jalankan dan catat exit code, implementasikan perubahan minimum, lalu jalankan verification lengkap.

Verification wajib: cargo test --workspace --all-targets.
Acceptance focus: corpus synthetic, reproducer disimpan, parser tidak panic, resource bounds dicatat.
...
- Expected files/interfaces: core property tests; fuzz targets/corpus documentation; CI bounded fuzz smoke.
- Expected behavior: Tambahkan bounded property/fuzz targets untuk state, canonical path, manifest, package containment, audit continuity, dan verifier input.
- Explicit non-goals: fitur setelah Day 22, production claim, raw/elevated operation tanpa gate, refactor di luar scope, dan perubahan RFC.
...
```text
Anda adalah independent reviewer Codex untuk M0-D22. Jangan menulis pada worktree author. Baca RFC, active runbook, commit_before, frozen commit_after, diff, test logs, dan local evidence.

Verifikasi sendiri scope, correctness, negative paths, false-complete resistance, forensic validity, security, dependency boundary, deterministic behavior, performance equivalence, documentation, dan test oracle. Fokus khusus: corpus synthetic, reproducer disimpan, parser tidak panic, resource bounds dicatat.

Jalankan command aman yang relevan: cargo test --workspace --all-targets. Bandingkan exact local/remote/PR/CI SHA bila remote evidence tersedia. SHA mismatch adalah BLOCKED + REMOTE-STATE-DIVERGENCE. Hasil tanpa reproducible evidence adalah UNVERIFIED.
...
Jika semua exit checklist lulus, handoff ke `DAY-23.md` dengan frozen SHA, TaskResult.v1 author/reviewer/Antigravity, CI/platform evidence, incident/recovery count, dan remaining risks. Selain itu tetap pada Day 22.
