# Day 30 — Production-Directed EAC dan fsnap v0.1 freeze

## Outcome

Audit seluruh evidence dan klasifikasikan hanya Production-Directed Engineering Alpha Candidate — Lab Use Only; bekukan `.fsnap` v0.1 untuk Analysis bila semua compatibility gate lulus.

## Entry Gate

- Prasyarat: Day 29 dengan independent review EXPECTED_PASS.
- Project root: `/Users/user/Projects/Trareon/Trareon Acquire`.
- Branch: `feat/m0-d30-gate`; satu author dan satu worktree aktif.
...
- [ ] Pastikan Issue Day 30 berada di status yang benar dan tidak ada Day lain menulis file yang sama.
- [ ] Catat commit_before, branch, worktree, dan recovery count.
- [ ] Salin Author Prompt hanya ke Codex.
- [ ] Setelah author membekukan commit, salin Reviewer Prompt hanya ke Human.
...
```text
Anda adalah author untuk M0-D30 Trareon Acquire.

Outcome: Audit seluruh evidence dan klasifikasikan hanya Production-Directed Engineering Alpha Candidate — Lab Use Only; bekukan `.fsnap` v0.1 untuk Analysis bila semua compatibility gate lulus.
Scope file/interface: evidence index; milestone review; risk register; capability matrix; fsnap contract/golden fixtures.
Branch wajib: feat/m0-d30-gate. Baca authority files dan existing code sebelum perubahan. Gunakan TDD: tulis test yang gagal dengan alasan yang diharapkan, jalankan dan catat exit code, implementasikan perubahan minimum, lalu jalankan verification lengkap.

Verification wajib: cargo test --workspace --all-targets; npm run check; npm run build; sh scripts/validate-ai-operations.sh; shasum -a 256 -c docs/RFC-BASELINE.sha256.
Acceptance focus: zero open P0/P1, exact-SHA evidence lengkap, human approval; tidak publish/release/sign.
...
- Expected files/interfaces: evidence index; milestone review; risk register; capability matrix; fsnap contract/golden fixtures.
- Expected behavior: Audit seluruh evidence dan klasifikasikan hanya Production-Directed Engineering Alpha Candidate — Lab Use Only; bekukan `.fsnap` v0.1 untuk Analysis bila semua compatibility gate lulus.
- Explicit non-goals: fitur setelah Day 30, production claim, raw/elevated operation tanpa gate, refactor di luar scope, dan perubahan RFC.
- Diff di luar map menjadi `FIX + SCOPE-DRIFT` sampai dijelaskan dan disetujui.

## Expected Result

- Verification: `cargo test --workspace --all-targets; npm run check; npm run build; sh scripts/validate-ai-operations.sh; shasum -a 256 -c docs/RFC-BASELINE.sha256`.
...
```text
Anda adalah independent reviewer Human untuk M0-D30. Jangan menulis pada worktree author. Baca RFC, active runbook, commit_before, frozen commit_after, diff, test logs, dan local evidence.

Verifikasi sendiri scope, correctness, negative paths, false-complete resistance, forensic validity, security, dependency boundary, deterministic behavior, performance equivalence, documentation, dan test oracle. Fokus khusus: zero open P0/P1, exact-SHA evidence lengkap, human approval; tidak publish/release/sign.

Jalankan command aman yang relevan: cargo test --workspace --all-targets; npm run check; npm run build; sh scripts/validate-ai-operations.sh; shasum -a 256 -c docs/RFC-BASELINE.sha256. Bandingkan exact local/remote/PR/CI SHA bila remote evidence tersedia. SHA mismatch adalah BLOCKED + REMOTE-STATE-DIVERGENCE. Hasil tanpa reproducible evidence adalah UNVERIFIED.
...
```text
Validasi frozen SHA untuk mode VALIDATE_MILESTONE_DISPLAY. Jangan mengubah core semantics. Uji alur, keyboard/focus, error/limitation visibility, viewport, dan copy yang relevan. Simpan screenshot teredaksi tanpa data pribadi. Laporkan mismatch sebagai FIX; jangan memperbaiki branch author. Akhiri dengan TaskResult.v1.
...
Jika semua exit checklist lulus, handoff ke `M1 rolling-wave decision` dengan frozen SHA, TaskResult.v1 author/reviewer/Antigravity, CI/platform evidence, incident/recovery count, dan remaining risks. Selain itu tetap pada Day 30.
