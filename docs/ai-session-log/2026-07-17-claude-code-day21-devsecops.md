...
- **`.github/workflows/ci.yml`** (modified): new `security` job — `cargo deny check` (advisories/bans/licenses/sources), `npm audit --omit=dev --prefix apps/trareon-acquire`, a committed-secret pattern grep (private key blocks, AWS access key IDs), and a `cargo metadata`-derived dependency manifest uploaded as a CI artifact, explicitly labeled provenance-intent rather than a full CycloneDX/SPDX SBOM.
...
- `npm audit --omit=dev --prefix apps/trareon-acquire` (0 vulnerabilities), the secret-pattern grep (no matches), and `cargo metadata --locked --format-version 1` (succeeds) — all run locally before writing the CI job, so the CI job was expected to pass rather than being iterated on blindly.
